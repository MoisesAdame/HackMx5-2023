...
require "connect.php";
$queryEmail = "SELECT participantes.idParticipante, participantes.correo, participantes.nombre FROM participantes INNER JOIN confirmacion_datos ON participantes.idParticipante = confirmacion_datos.idParticipante WHERE confirmacion_datos.recordatorio = 0";
...
if($queryEmail->num_rows != 0)
{
  while($row  = $queryEmail->fetch_assoc())
  {
    ...
    try {
          ...
          $body = file_get_contents('view/mailRecordatorio.php', true);
          $body = str_replace("[NOMBRE_USUARIO]",$row["nombre"],$body);
          $body = str_replace("[ID_USUARIO]",$row["idParticipante"],$body);
          $mail->Body    = $body;
          $mail->AltBody = "\nHola! ".$row["nombre"].", recuerda completar tu registro y registrar a tu equipo en: http://hackmx.mx/?miRegistro \nNúmero de Registro: " .$row["idParticipante"];
          $mail->send();
          echo "Mail enviado a ".$row["idParticipante"]."<br>";
          $query2 = "UPDATE confirmacion_datos SET recordatorio = 1 WHERE idParticipante = '".$row["idParticipante"]."'";
